<%@ Page Language="C#" AutoEventWireup="true" CodeFile="bacOnClarifications.aspx.cs" Inherits="web_buyer_screens_bacOnClarifications" %>
                                        ...
                                        <table border="0" cellpadding="0" cellspacing="0" width="100%" id="table4">
                                            <tr>
                                                <td>
                                                    <h1>
                                                        <br />
                                                        BAC under Clarifications
                                                    </h1>
                                                    <p>
                                                        These are your BAC under clarifications. Click on the bid event reference number 
                                                        or the bid event description to view the details of BAC.<br />
                                                    ...
                                                    <asp:GridView ID="gvBids" runat="server" SkinID="BidEvents" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False" OnRowCommand="gvBids_RowCommand" DataKeyNames="BidRefNo" DataSourceID="dsAproved">
                                                        <Columns>
                                                            ...
                                                            <%--<asp:TemplateField HeaderText="Clarified To" SortExpression="ToUserId">
                                                                <HeaderStyle HorizontalAlign="Center" />
                                                                <ItemStyle HorizontalAlign="Center" Width="180px" />
                                                                <ItemTemplate>
                                                                    &nbsp;<asp:Label ID="Label3" runat="server" Text='<%# Bind("ToUserId") %>'></asp:Label>
                                                                </ItemTemplate>
                                                            </asp:TemplateField>--%>
                                                    ...
                                                    <asp:SqlDataSource ID="dsAproved" runat="server" ConnectionString="<%$ ConnectionStrings:EBidConnectionString %>"
                                                        SelectCommand="SELECT DISTINCT b.BacRefNo, b.BidRefNo, b.ItemDesc, b.PreparedDt, b.DateSubmitted, b.ClarifyDt_1, c.ToUserId, CONVERT(VARCHAR,b. BidRefNo)+';'+CONVERT(VARCHAR, b.BacRefNo)+';'+CONVERT(VARCHAR, c.FrUserId) AS BACBID FROM tblBacBidItems b
                                                            INNER JOIN (
                                                            SELECT * FROM tblBACClarifications d WHERE d.datePosted=(select max(e.datePosted) FROM tblBACClarifications e WHERE e.BidRefNo=d.BidRefNo)) c ON b.BidRefNo=c.BidRefNo AND c.ToUserId<>@BuyerId  WHERE b.Status=@Status AND   b.BuyerId = @BuyerId ORDER BY b.DateSubmitted DESC"
