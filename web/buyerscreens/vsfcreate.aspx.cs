...
public partial class web_buyerscreens_vsfcreate : System.Web.UI.Page
{
    ...
    {
        if (Session["Userid"] == null) Response.Redirect("../unauthorizedaccess.aspx");
        FormsAuthenticationHelper.AuthenticateUserWithReturnUrl();
        if (Int32.Parse(Session[Constant.SESSION_USERTYPE].ToString().Trim()) != (int)Constant.USERTYPE.BUYER)
            Response.Redirect("../unauthorizedaccess.aspx");

        //TestShowAllSessions();
        if (Session["pageDetails"] == null || Session["pageDetails"].ToString() == "")
        {
            Session["pageDetails"] = "index.aspx";
        }
        if (Session["VSFId"] == null || Session["VSFId"].ToString() == "" || Request.QueryString["create"] == "new")
        {
            SaveToDB_Init();
        }
        if (IsPostBack)
    ...
    void SaveToDB()
    {
        ...
        else if (Request.Form["__EVENTTARGET"] == "txtVendorName")
        {
            ...
            txtVendorName.DataBind();
            repeaterVSFDetails.DataBind();
            //txtVendorName_DataBound();
        }
        else
        {
            ...
            foreach (RepeaterItem oItem in oRpt.Items)
            {
                ...
                iSelected = Selected.Checked ? 1 : 0;
                ...
                using (conn = new SqlConnection(connstring))
                {
                    using (cmd = new SqlCommand(query, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@Selected", iSelected);
                        conn.Open(); cmd.ExecuteNonQuery();
                    }
                }
            }
    ...
    protected void repeaterVSFDetails_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            if ((((DataRowView)e.Item.DataItem)["VendorName"].ToString()).Contains("WITH ISSUE"))
            {
                //itemInList.Attributes.Add("style", "background-color:#FFBE93;border-top:1px solid #fff;");
                string alertThis = "";
                string tooltipThis = "";
                //query = "SELECT t1.*, t2.VendorName FROM tblIssue t1, tblVendors t2 WHERE t2.VendorId=t1.VendorId AND t1.VendorId=@VendorId";
                query = "s3p_EBid_SearchVendorIdWithIssue"; //##storedProcedure
                using (conn = new SqlConnection(connstring))
                {
                    using (cmd = new SqlCommand(query, conn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure; //##storedProcedure
                        cmd.Parameters.AddWithValue("@VendorId", Convert.ToInt32(((DataRowView)e.Item.DataItem)["VendorId"].ToString()));
                        conn.Open();
                        //Process results
                        oReader = cmd.ExecuteReader();
                        if (oReader.HasRows)
                ...
                //Response.Write(alertThis);
                //Response.Write(((DataRowView)e.Item.DataItem)["VendorName"].ToString());
                ((Label)e.Item.FindControl("VendorNameLbl")).Text = "<a href=\"javascript:alert('" + alertThis + "');\" style=\"color:#f60\">" + ((DataRowView)e.Item.DataItem)["VendorName"].ToString() + "</a>";
            }
        }
    }

    protected void txtVendorName_DataBound(object sender, EventArgs e)
    {

        foreach (ListItem itemInList in txtVendorName.Items)
        {
