...
public partial class web_buyerscreens_bidawardingchecklistclarify : System.Web.UI.Page
{
    ...
    void SaveToDB()
    {
        ...
        sCommand = sCommand + " SS_NoBidders=" + ss_nobidders + ", SS_NoBidsRcvd=" + ss_nobidsrcvd + ", SS_TechCompliance=" + ss_techcompliance + ", Status=" + status1 + ",  SavingsGen_Amount=" + lblNegDisc + ", SavingsGen_PctSpend=" + lblPercentSpend + ", descPaymentTerms='" + Request.Form["descPaymentTerms"].ToString().Replace("'", "''") + "', ss_crc_no='" + Request.Form["ss_crcnumber"].Replace("'", "''") + "', LowestPriceBidder=" + chkLowestPriceBidder + ", BidRefNoParent = " + Request.Form["BuyerBidForBacParent"] + "   WHERE BidRefNo =" + Session["BuyerBidForBac"];
        ...
        sCommand = "SELECT t1.PurchasingID, t2.LastName + ', ' + t2.FirstName + ' ' + t2.MiddleName AS Name1 FROM tblSupervisor t1  INNER JOIN tblPurchasing t2 ON t1.PurchasingID=t2.PurchasingID WHERE BuyerID=" + Session["UserId"];
        oReader = SqlHelper.ExecuteReader(connstring, CommandType.Text, sCommand);
        if (oReader.HasRows)
        {
            oReader.Read();
    ...
    protected void doClarify()
    {
        ...
        //if(Request.Form["__EVENTARGUMENT"].ToString()=="Clarify")
        ...
        else
        {
           ...
           // sCommand = sCommand + Session["BuyerBidForBac"].ToString() + ", " + Session["UserId"].ToString() + ", '" + Request.Form["txtClarify"].ToString().Replace("'","''") + "', GETDATE())";
           //SqlHelper.ExecuteNonQuery(connstring, CommandType.Text, sCommand);
    ...
    protected void doBuyerBidForBac()
    {
        ...
        iApproverCount = 0;
        //dsApprover.SelectCommand = "IF EXISTS (SELECT 1 FROM tblBACSupportingDocuments WHERE BidRefNo=" + Session["BuyerBidForBac"] + " AND DocuName='bfa_LowestPr') BEGIN SELECT t1.BACId, t1.Initials AS Name1, t1.LimitLowestChar AS ApprovingMaxLimit, t1.LimitLowestTo, t1.LimitNonLowestTo, t1.Committee, CASE WHEN t2.Approver IS NULL THEN 0 ELSE 1 END AS isApprover, t2.ApprovedDt FROM vwApprovers t1 LEFT OUTER JOIN vmBacBidApprovers t2 on t1.BACId = t2.Approver AND BidRefNo =" + Session["BuyerBidForBac"] + "  WHERE t1.Approver = 0 AND t1.OIC = 0 AND t1.LimitLowest <= (SELECT SUM(TotalCost) FROM tblBACEvaluationDetails WHERE BidRefNo=" + Session["BuyerBidForBac"] + " AND Chkd=1) UNION SELECT t1.BACId, t1.Initials AS Name1, t1.LimitLowestChar AS ApprovingMaxLimit, t1.LimitLowestTo, t1.LimitNonLowestTo, t1.Committee, CASE WHEN t2.Approver IS NULL THEN 0 ELSE 1 END AS isApprover, t2.ApprovedDt FROM vwApprovers t1 LEFT OUTER JOIN vmBacBidApprovers t2 on t1.BACId = t2.Approver AND BidRefNo =" + Session["BuyerBidForBac"] + "  WHERE t1.Approver = 1 AND t1.OIC = 0 AND (SELECT SUM(TotalCost) FROM tblBACEvaluationDetails WHERE BidRefNo=" + Session["BuyerBidForBac"] + " AND Chkd=1) BETWEEN t1.LimitLowest AND t1.LimitLowestTo ORDER BY LimitLowestTo ASC, Committee, Name1 END ELSE BEGIN SELECT t1.BACId, t1.Initials AS Name1, t1.LimitNonLowestChar AS ApprovingMaxLimit, t1.LimitLowestTo, t1.LimitNonLowestTo, t1.Committee, CASE WHEN t2.Approver IS NULL THEN 0 ELSE 1 END AS isApprover, t2.ApprovedDt FROM vwApprovers t1 LEFT OUTER JOIN vmBacBidApprovers t2 on t1.BACId = t2.Approver AND BidRefNo =" + Session["BuyerBidForBac"] + " WHERE t1.Approver = 0 AND t1.OIC = 0 AND t1.LimitNonLowest <= (SELECT SUM(TotalCost) FROM tblBACEvaluationDetails WHERE BidRefNo=" + Session["BuyerBidForBac"] + " AND Chkd=1) UNION SELECT t1.BACId, t1.Initials AS Name1, t1.LimitNonLowestChar AS ApprovingMaxLimit, t1.LimitLowestTo, t1.LimitNonLowestTo, t1.Committee, CASE WHEN t2.Approver IS NULL THEN 0 ELSE 1 END AS isApprover, t2.ApprovedDt FROM vwApprovers t1 LEFT OUTER JOIN vmBacBidApprovers t2 on t1.BACId = t2.Approver AND BidRefNo =" + Session["BuyerBidForBac"] + " WHERE t1.Approver = 1 AND t1.OIC = 0 AND (SELECT SUM(TotalCost) FROM tblBACEvaluationDetails WHERE BidRefNo=" + Session["BuyerBidForBac"] + " AND Chkd=1) BETWEEN t1.LimitNonLowest AND t1.LimitNonLowestTo ORDER BY LimitNonLowestTo ASC, LimitLowestTo ASC, Committee, Name1 END";
    ...
    private string NextApprover()
    {
        ...
        sCommand = "SELECT TOP 1 * FROM (SELECT " + Session["BuyerBidForBac"] + " as BidRefNo, t1.PurchasingID BACId, t2.LastName + ', ' + t2.FirstName + ' ' + t2.MiddleName AS Name1, (SELECT ApprovedDt_0 FROM tblBacBidItems WHERE BidRefNo = " + Session["BuyerBidForBac"] + ") as ApprovedDt, 0 ApprovingLimit FROM tblSupervisor t1 INNER JOIN tblPurchasing t2 ON t1.PurchasingID=t2.PurchasingID WHERE BuyerID=" + Session["UserId"];
    ...
    protected void ShowApprover()
    {
        ...
        RespondTo.Value = (Arr.Length < 5) ? null : Arr[1].ToString();
        //RespondTo.Value = Arr[1].ToString();
	if (Arr.Length == 5)
	{
        	if (ClarifyTo.Value.ToString() == Arr[1].ToString())
        	{
            		//btnClarify.Visible = false;
        	}
        }

        //Response.Write(NextApprover());
        //sCommand = "select PurchasingID, Name1 FROM ( ";
        //sCommand = sCommand + "SELECT t1.PurchasingID, t2.LastName + ', ' + t2.FirstName + ' ' + t2.MiddleName AS Name1 FROM tblSupervisor t1 ";
        //sCommand = sCommand + "INNER JOIN tblPurchasing t2 ON t1.PurchasingID=t2.PurchasingID WHERE BuyerID=" + Session["UserId"] + " ";
        //sCommand = sCommand + "UNION ";
        //sCommand = sCommand + "SELECT t1.BacID PurchasingID, t1.LastName + ', ' + t1.FirstName + ' ' + t1.MiddleName AS Name1 FROM tblBidAwardingCommittee t1 ) as a1 INNER JOIN (SELECT TOP 1 * FROM tblBACClarifications d  ";
        //sCommand = sCommand + "WHERE d.ToUserId = " + Session["UserId"] + "  ANd d.BidRefNo = " + Session["BuyerBidForBac"] + " ORDER BY d.DatePosted desc) t2 ON t2.FrUserId =a1.PurchasingID ";

        //if (NextApprover() == "" || NextApprover() == "ApprovedDt_00")
        //{
        //    //sCommand = "SELECT t1.PurchasingID, t2.LastName + ', ' + t2.FirstName + ' ' + t2.MiddleName AS Name1 FROM tblSupervisor t1 INNER JOIN tblPurchasing t2 ON t1.PurchasingID=t2.PurchasingID WHERE BuyerID=" + Session["UserId"];
        ...
        //{
        //    oReader.Read();
        //    RespondTo.Value = oReader["Name1"].ToString();
        //}
        //oReader.Close();

        // CLARIFY TO:
        //sCommand = "select PurchasingID, Name1 FROM (SELECT t1.PurchasingID, t2.LastName + ', ' + t2.FirstName + ' ' + t2.MiddleName AS Name1 FROM tblSupervisor t1 INNER JOIN tblPurchasing t2 ON t1.PurchasingID=t2.PurchasingID WHERE BuyerID=" + Session["UserId"] + ") t2 ON t2.FrUserId =a1.PurchasingID ";
        //oReader = SqlHelper.ExecuteReader(connstring, CommandType.Text, sCommand);
        //if (oReader.HasRows)
        //{
        //    oReader.Read();
    ...
    protected void BuyerBidForBac_DataBound(object sender, EventArgs e)
    {
        ...
        lblBuyerBidForBac.Text = Session["BuyerBidForBac"].ToString();
    }
    ...
    private string CreateNotificationBodyClarify(string FromName1, string ToName1)
    {
        ...
        //sCommand = "select PurchasingID, Name1, EmailAdd FROM ( SELECT t1.PurchasingID, t2.LastName + ', ' + t2.FirstName + ' ' + t2.MiddleName AS Name1, t2.EmailAdd FROM tblSupervisor t1 INNER JOIN tblPurchasing t2 ON t1.PurchasingID=t2.PurchasingID WHERE BuyerID=" + Session["UserId"] + " UNION SELECT t1.BacID PurchasingID, t1.LastName + ', ' + t1.FirstName + ' ' + t1.MiddleName AS Name1, t1.EmailAdd FROM tblBidAwardingCommittee t1 ) as a1 INNER JOIN (SELECT TOP 1 * FROM tblBACClarifications d  WHERE d.ToUserId = " + Session["UserId"] + "  ANd d.BidRefNo = " + Session["BuyerBidForBac"] + " ORDER BY d.DatePosted desc) t2 ON t2.FrUserId =a1.PurchasingID ";
        //oReader = SqlHelper.ExecuteReader(connstring, CommandType.Text, sCommand);
        //if (oReader.HasRows)
        //{
        //    oReader.Read();
