<%@ Page Language="C#" AutoEventWireup="true" CodeFile="bacforclarifications.aspx.cs" Inherits="web_buyer_screens_ApprovedBidItems" %>
                                        ...
                                        <table border="0" cellpadding="0" cellspacing="0" width="100%" id="table4">
                                            <tr>
                                                <td>
                                                    <h1>
                                                        <br />
                                                        BAC for Clarifications
                                                    </h1>
                                                    <p>
                                                        These are your BAC for clarifications. Click on the bid event reference number 
                                                    ...
                                                    <asp:SqlDataSource ID="dsAproved" runat="server" ConnectionString="<%$ ConnectionStrings:EBidConnectionString %>"
                                                        SelectCommand="SELECT DISTINCT b.BacRefNo, b.BidRefNo, b.ItemDesc, b.PreparedDt, b.DateSubmitted, b.ClarifyDt_1, c.ToUserId, CONVERT(VARCHAR,b. BidRefNo)+';'+CONVERT(VARCHAR, b.BacRefNo)+';'+CONVERT(VARCHAR, c.FrUserId) AS BACBID FROM tblBacBidItems b
                                                            INNER JOIN (
                                                            SELECT * FROM tblBACClarifications d WHERE d.ToUserId=@BuyerId AND d.datePosted=(select max(e.datePosted) FROM tblBACClarifications e WHERE e.BidRefNo=d.BidRefNo)) c ON b.BidRefNo=c.BidRefNo AND c.ToUserId=@BuyerId 
