...
{
    public partial class bidseventsforawarding : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            PageTitle.InnerText = String.Format(Constant.TITLEFORMAT, "Bid Events For Awarding");
        }

        protected void gvBidEventsForAwarding_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName.Equals("openEvent"))
            {
                try
                {
                    ...
                    gvBidEventsForAwarding.DataBind();
        ...
        protected void gvBidEventsForAwarding_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                
                string key1 = gvBidEventsForAwarding.DataKeys[e.Row.RowIndex].Values[0].ToString();
                string key2 = gvBidEventsForAwarding.DataKeys[e.Row.RowIndex].Values[1].ToString();
                string key3 = gvBidEventsForAwarding.DataKeys[e.Row.RowIndex].Values[2].ToString();
                string key4 = gvBidEventsForAwarding.DataKeys[e.Row.RowIndex].Values[3].ToString();
                string key5 = gvBidEventsForAwarding.DataKeys[e.Row.RowIndex].Values[4].ToString();
                string key6 = gvBidEventsForAwarding.DataKeys[e.Row.RowIndex].Values[5].ToString();
               
            }
            if (e.Row.RowType == DataControlRowType.Footer)
            {
                if (gvBidEventsForAwarding.DataKeys.Count > 0)
                {
                    //// Use the GetEnumerator method to create an enumerator that 
                    //// contains the DataKey objects for the GridView control.
                    //IEnumerator keyEnumerator = gvBidEventsForAwarding.DataKeys.GetEnumerator();
                    ...
                    //    Panel pnl1 = (Panel)gvBidEventsForAwarding.Rows[i].Cells[3].Controls[1];
                    //    Panel pnl2 = (Panel)gvBidEventsForAwarding.Rows[i].Cells[3].Controls[3];
                    //    LinkButton lb = (LinkButton)gvBidEventsForAwarding.Rows[i].Cells[3].Controls[3].Controls[1];
